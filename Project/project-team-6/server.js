...
app.get('/home', (req, res) => {
  if (req.session.user) {
    ...
    var hs = [];  //Used to merge users and hs table into one array
    Highscore.findAll().then(highscores => {
      //add sort method
      highscores.forEach(element => {
        User.findOne({ where: { id: element.username_id } }).then(_user => {
          hs.push({ username: _user.username, score: element.score, date: element.date });
        });
      });
      if (hs.length > 0) {
